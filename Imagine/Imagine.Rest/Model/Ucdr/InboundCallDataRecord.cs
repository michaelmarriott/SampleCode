...
namespace Imagine.Rest.Model.Dr {
  public class InboundCallDataRecord : CallDataRecord<InboundCallDataRecord> {
    ...
    public List<InboundCallDataRecord> Find(int lobId, string customerAccountId, string[] deviceId, DateTime dateStart, DateTime dateEnd, int limit, int offSet) {
      var dataSet = GetData("GET_INCOMING_CDRS_ACCOUNT", lobId, customerAccountId, deviceId, dateStart, dateEnd, limit, offSet);
    ...
    /// <returns></returns>
    private List<InboundCallDataRecord> PopulateCdr(DataSet data) {
      var cdrCallRecordsView = new List<InboundCallDataRecord>();
      if (data != null && data.Tables != null && data.Tables.Count > 0) {
        for (int i = 0; i < data.Tables[0].Rows.Count; i++) {
          var cdrCallRecord = new InboundCallDataRecord();
          cdrCallRecord.Account = data.Tables[0].Rows[i][0].ToString();
          cdrCallRecord.BillCost = double.Parse(data.Tables[0].Rows[i][1].ToString()); //"Bill Cost"
          cdrCallRecord.DialledNumber = data.Tables[0].Rows[i][2].ToString();//"Phone Number"
          cdrCallRecord.DeviceId = data.Tables[0].Rows[i][3].ToString();
          cdrCallRecord.Seconds = int.Parse(data.Tables[0].Rows[i][4].ToString()); //"Seconds"
          cdrCallRecord.Minutes = double.Parse(data.Tables[0].Rows[i][5].ToString()); //"Minutes"
          // cdrCallRecord.TrunkId = data.Tables[0].Rows[i][1].ToString(); //Call Type"
          cdrCallRecord.CallType = data.Tables[0].Rows[i][6].ToString(); //Call Type"
          cdrCallRecord.TimeCategory = data.Tables[0].Rows[i][7].ToString(); //"Time Category"
          cdrCallRecord.CallSource = data.Tables[0].Rows[i][8].ToString(); //Call Source
          cdrCallRecord.CallConnected = data.Tables[0].Rows[i][9].ToString();//Start Time
          cdrCallRecord.Description = data.Tables[0].Rows[i][10].ToString();//Description
          cdrCallRecord.BillRunName = data.Tables[0].Rows[i][12].ToString();//BillRunName
          if (data.Tables[0].Rows[i][11].ToString() != "") {
            cdrCallRecord.BillRecordId = double.Parse(data.Tables[0].Rows[i][13].ToString());//BillRecordId
          }
          cdrCallRecord.ContractId = data.Tables[0].Rows[i][14].ToString();//ContractId
          cdrCallRecord.ActualCost = double.Parse(data.Tables[0].Rows[i][15].ToString());//ActualCost
        }
      }
