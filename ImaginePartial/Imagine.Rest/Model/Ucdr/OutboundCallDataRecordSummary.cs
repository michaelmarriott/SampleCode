...
namespace Imagine.Rest.Model.Dr {
  ...
  public class OutboundCallDataRecordSummary : CallDataRecord<OutboundCallDataRecordSummary> {
    ...
    internal List<OutboundCallDataRecordSummary> Find(int lobId, string customerAccountId, string[] deviceId, DateTime dateStart, DateTime dateEnd, int limit, int offSet) {
      var dataSet = GetData("GET_RATED_CDRS_SUM_ACCOUNT",lobId, customerAccountId, deviceId, dateStart, dateEnd, limit, offSet);
    ...
    /// <returns></returns>
    private List<OutboundCallDataRecordSummary> PopulateSummaryCdr(DataSet data) {
      var cdrCallRecordsSummaryView = new List<OutboundCallDataRecordSummary>();
      if (data != null && data.Tables != null && data.Tables.Count > 0) {
        for (int i = 0; i < data.Tables[0].Rows.Count; i++) {
          var cdrCallRecordSummary = new OutboundCallDataRecordSummary();
